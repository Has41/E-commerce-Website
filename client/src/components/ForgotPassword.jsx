  ...
  return (
    ...
    <section className='bg-slate-50 py-8 lg:py-28 max-w-full'>
        <div className='text-center bg-white py-8 lg:py-24 w-[90%] ml-4 sm:ml-8 sm:space-y-8 lg:ml-[360px] rounded-sm shadow-sm lg:w-[45%]'>
            <p className='uppercase font-mont lg:font-poppins lg:tracking-wider font-semibold text-black/80 text-xl lg:text-3xl lg:mb-8 lg:-mt-4'>Forgot Password</p>
            ...
            <form onSubmit={forgotPassword}>
                <div className='space-y-5 mt-4 lg:ml-0 sm:flex sm:flex-col sm:items-center sm:justify-center lg:flex lg:flex-col lg:items-center lg:justify-center'>
                  <input value={email} onChange={(e) => setEmail(e.target.value)} className='py-2 lg:py-3 rounded-sm border border-slate-300 w-[80%] sm:w-[85%]' type="email" placeholder='Enter Verification Email' />
                </div>
                <button type='submit' className='border-2 border-black/80 text-black/80 w-[80%] sm:w-[85%] h-[40px] sm:h-[40px] sm:font-semibold sm:text-lg font-mont mt-5 px-[93px] lg:h-[50px] hover:bg-black/80 hover:border hover:border-black/80 hover:text-white transition-all duration-500 py-1 lg:py-2 lg:text-lg lg:font-semibold rounded-sm uppercase'>
                {loading ? 
                <div className='flex items-center justify-center'>
                    <svg className="w-4 h-4 sm:w-5  sm:h-5 lg:w-5 lg:h-5 text-gray-200 animate-spin dark:text-gray-600 fill-white" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
